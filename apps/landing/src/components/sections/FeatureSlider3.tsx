...
export function FeatureSlider3({ heading = "Ámaxa Community Partners", subhead, items, perPage = 3 }: Props) {
      ...
      <div className="flex flex-col gap-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {visible.map((item) => (
            ...
            >
              {/* Image */}
              <div className="w-full h-[440px] rounded-[36px] bg-neutral-2-500 border-[5px] border-white overflow-hidden">
                {item.imageSrc && (
                  <img
                    src={item.imageSrc}
                    alt={item.title}
              ...
              <div className="flex flex-col gap-2">
                <p className="text-[24px] font-semibold text-white leading-snug">{item.title}</p>
                {item.subtitle && (
